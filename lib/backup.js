...
async function backupSQLite(src, dstDir) {
  ...
  ensureDir(dstDir);
  const out = `${backupFilename(src, dstDir)}.gz`;
  const unique = `${process.pid}-${Math.random().toString(16).slice(2)}`;
  const snapshot = `${out}.snapshot-${unique}`;
  const partial = `${out}.partial-${unique}`;
  let db;
  ...
  try {
    ...
    // Compress to a partial file, then rename it so an interrupted gzip cannot
    ...
    // Never leave a partial database or archive looking successful.
