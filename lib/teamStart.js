...
function getTeamStartLockReason(state = {}) {
  ...
  const legacyServerLocks =
    state.lockedServers instanceof Set && state.lockedServers.size > 0;
  const playerLocks =
    state.lockedPlayers instanceof Map && state.lockedPlayers.size > 0;
  const unifiedServerLocks =
    state.locks?.servers &&
    typeof state.locks.servers === "object" &&
    Object.keys(state.locks.servers).length > 0;

  return legacyServerLocks || playerLocks || unifiedServerLocks
    ? "match"
    : null;
}
