...
function editMap(index1, name, mirv, tier) {
  ...
  const prev = arr[idx];
  arr[idx] = normMap({ key: prev.key, name, mirv, tier });
  _writeJson(arr);
...
function pickTieredMapsWithCountsWeighted(allMaps, excludeNames = new Set(), carryOver = null) {
  ...
  // 🪄 Log results for sanity
  const summary = out.map(m => `${m.name} (T${m.tier}${m.name === carryMap?.name ? "⭐" : ""})`).join(", ");
  const counts = out.reduce((a, m) => { a[m.tier] = (a[m.tier] || 0) + 1; return a; }, {});
  console.log(`[maps DEBUG] weighted pick → ${summary}`);
  console.log(`[maps DEBUG] counts: T1=${counts[1] || 0}  T2=${counts[2] || 0}  T3=${counts[3] || 0}`);
