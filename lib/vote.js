...
function getMapVoteShortenReason({ eligible = [], voted = new Set(), counts = new Map(), options = [] }) {
  const realEligible = eligible.filter(uid => isRealDiscordId(uid));
  const waiting = realEligible.filter(uid => !voted.has(uid));
  const majorityThreshold = Math.floor(realEligible.length / 2) + 1;
  const majorityOptions = options.filter(option =>
    (counts.get(option.id) || 0) >= majorityThreshold
  );

  if (majorityOptions.length) {
    return { type: "majority", options: majorityOptions, realEligible, waiting };
  }
  if (waiting.length === 1) {
    return { type: "one_remaining", options: [], realEligible, waiting };
  }
  if (waiting.length === 0) {
    return { type: "all_voted", options: [], realEligible, waiting };
  }
  return null;
}
...
async function startVote(state, message, cfg) {
  ...
  const roster = state.queueSnapshot || state.queue.slice(0, state.MAX_PLAYERS);
  const playerNames = new Map(
    roster.map(p => [
      String(p.id),
      getStoredPlayerName(cfg.elo, p.id, p.name),
    ])
  );
  ...
  function getWaitingVoters() {
    return handle.eligible.filter(uid => {
      if (!isRealDiscordId(uid) || uid.startsWith("test_")) return false;
      return !handle.votedByUser.has(uid);
    });
  }

  function waitingMessage(initial = false) {
    const waiting = getWaitingVoters();
    if (!waiting.length) {
      return {
        content: "✅ **Everyone has voted!**",
        allowedMentions: { parse: [] },
      };
    }

    const mentions = waiting.map(id => `<@${id}>`).join(" ");
    return {
      content: initial
        ? `${mentions} — vote is now live!`
        : `⏳ **Still waiting on:** ${mentions}`,
      allowedMentions: { users: waiting },
    };
  }

  function winnerAnnouncement(option) {
    if (!option) return null;
    if (handle.kind === "server") {
      return `🖥️ Server chosen: **${option.name}**`;
    }
    if (!/new\s*maps?/i.test(option.name || "")) {
      return `🗺️ Map chosen: **${option.name}**`;
    }
    return `🆕 **New Maps** selected — loading second vote...`;
  }

  async function postWinnerAnnouncement(option) {
    const announcement = winnerAnnouncement(option);
    if (!announcement) return;
    await message.channel.send(announcement).catch(e => {
  ...
  if (handle.eligible?.length) {
    handle.notificationMessage = await message.channel.send(waitingMessage(true));
  }
  ...
  collector.on("collect", async (interaction) => {
    ...
    try {
      ...
      if (handle.notificationMessage) {
        await handle.notificationMessage.edit(waitingMessage());
      }
      await handle.message.edit({
        content: status,
        embeds: [buildEmbed()],
	...
	// ======================================================
	async function runFancySpinner(message, maps, cfg = {}) {
	  ...
	  const spinnerDurationMs = Number(cfg.spinnerDuration) * 1000;
	  const spinnerStepMs = Number.isFinite(spinnerDurationMs) && spinnerDurationMs > 0
	    ? Math.max(250, Math.round(spinnerDurationMs / 4.25))
	    : 800;
	  const randomFrame = () => Array.from(
		{ length: 3 },
	  ...
	  // Spin all three reels, then lock them from left to right. The question
	  // marks are the stopped reels; only the final reel lands on the winner.
	  const firstSpin = randomFrame();
	  const secondSpin = randomFrame();
	  const spinMsg = await message.channel.send({
		embeds: [
		  new EmbedBuilder()
			.setColor(0x5865f2)
			.setDescription(box(...firstSpin))
		],
	  });

	  await pause(spinnerStepMs);
	  await spinMsg.edit({
		embeds: [
		  new EmbedBuilder()
			.setColor(0xed4245)
			.setDescription(box(...secondSpin))
		],
	  });
	  ...
	  await spinMsg.edit({
		embeds: [
		  new EmbedBuilder()
			.setColor(0x5865f2)
			.setDescription(box("?", ...randomFrame().slice(1)))
		],
	  });
	  ...
	  await spinMsg.edit({
		embeds: [
		  new EmbedBuilder()
			.setColor(0xed4245)
			.setDescription(box("?", "?", randomFrame()[2]))
		],
	  });

	  await pause(Math.round(spinnerStepMs * 1.25));

	// The final spinner frame must reveal the selected option on all reels.
	// Do not leave the message on the intermediate `? ? 2` frame.
	const labelIcon =
	...
	try {
	  await spinMsg.edit({ content: "", embeds: [finalEmbed], components: [] });
	} catch (error) {
	  // If the final edit is rejected after the animation, post the exact same
	  // final frame so the tiebreaker cannot remain stuck on `? ? 2`.
	  console.error(`[vote.js] final ${cfg?.kind || ""} spinner edit failed:`, error);
	  await message.channel.send({ content: "", embeds: [finalEmbed] }).catch(fallbackError => {
	    console.error(`[vote.js] final ${cfg?.kind || ""} spinner fallback failed:`, fallbackError);
	  });
  }
  ...
  collector.on("end", async () => {
    ...
    if (topOptions.length > 1) {
      ...
      const spinnerOptions = topOptions.flatMap(o => {
		...
		const winner = await runFancySpinner(message, spinnerOptions, {
		  kind: handle.kind,
		  spinnerDuration: cfg.spinnerDuration,
		});
      best = winner;

      // The spinner is an edited animation, so also post a persistent winner
      // message before the next stage consumes the selected map/server.
      if (handle.cancelled || state.vote !== handle) return;
      postWinnerAnnouncement(best);
...
module.exports = { startVote, getMapVoteShortenReason };
