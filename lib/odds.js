...
const nameOf = (p) => String(p.name ?? p.display_name ?? p.username ?? p.tag ?? p.id ?? "Unknown");
...
function getRating(elo, p) {
  try {
    ...
    if (row && row.rating) return Number(row.rating);

    // fallback to display_name
    const row2 = elo?.db?.prepare("SELECT rating FROM ratings WHERE display_name = ?").get(nameOf(p));
    if (row2 && row2.rating) return Number(row2.rating);
...
function buildTeamScenariosEmbed({
  ...
  scenarios.forEach((s, i) => {
    ...
    const blueNames = s.blue.map(p => `${nameOf(p)} (${getRating(elo, p)})`).join(", ");
    const redNames  = s.red .map(p => `${nameOf(p)} (${getRating(elo, p)})`).join(", ");
