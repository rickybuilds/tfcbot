...
const { normalizedRating } = require("./ranks");

const NUM_EMOJI = ["", "1️⃣","2️⃣","3️⃣","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣","9️⃣"];
const NUM_SHORT = ["", ":one:", ":two:", ":three:", ":four:", ":five:", ":six:", ":seven:", ":eight:", ":nine:"];
const RANK_EMOJIS = [
  { min: 301,  max: 720,  emoji: "<:rank1:1410287215296254114>" },
...
function getRankEmoji(rating) {
  const r = normalizedRating(rating);
...
function formatPlayerName(state, elo, userId, fallbackName, privacy, showRank = true) {
  ...
  try {
    rating = Math.round(
      Number(elo.getRating(id, displayName, { createIfMissing: false }))
    );
  } catch {
    rating = 1941;
  }
